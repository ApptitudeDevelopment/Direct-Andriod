...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void saveUserData() {
        ...
        if (ImageUri == null)                // NO PROFILE IMAGE
        {
            userRef.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).hasChild("image"))    // check if user image exist so no replacement
                    {
            ...
            uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                ...
                public Task<Uri> then(@NonNull Task<UploadTask.TaskSnapshot> task) throws Exception {
                    ...
                    if (!task.isSuccessful()) {
                        throw task.getException();                                                    //profile image upload
                    }
                        ...
                        userRef.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).updateChildren(profileMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    Toast.makeText(SettingsActivity.this, "Profile Updated!", Toast.LENGTH_SHORT).show();    //succesfull update
                                }
                            }
                        });
                    }
